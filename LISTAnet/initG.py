...
for epoch in range(args.num_epochs):
    ...
    with tqdm(total=(lenDatSet - lenDatSet % args.batch_size), ncols=80) as t:
        ...
        for dummIn in range(numBatch):
            ...
            stdLF=torch.sqrt((torch.sum((volHat)**2,dim=(1,2,3))).view(volHat.shape[0],1,1,1)) # Norm of volume
            volHat_n=(volHat.clone())/(stdLF+_eps)

            dc_loss=(((volHat_n-volBatch))**2).mean() # Loss measured on volumes
            loss1=1e5*dc_loss # Total loss used for training (summation of loss on 3D volume, LF, adversarial)
